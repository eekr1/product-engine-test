...
- **Approved Input Örneği Göretimi:** Tamamlanmış, tutarlı, canonical intake standartlarına uygun ve üretime hazır bir girdi belgesinin yapısını sergilemek.
...
- **`input_id` Standardı:** `INPUT-<DESCRIPTOR>-V<N>` (Örn: `INPUT-WEB-APP-MINIMAL-001-V1`).
- **`status` Ayrımı:**
