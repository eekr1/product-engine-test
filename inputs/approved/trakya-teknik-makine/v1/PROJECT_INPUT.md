...
> [!IMPORTANT]
> Bu belge Product Engine'in canonical approved project truth girdisidir. Kullanıcı açık onayı ile (explicit user approval) dondurulmuştur.
...
- **ASM-01**: Language | `tr` | safe | Proje adı ve kaynak metinler Türkçe olduğu için birincil dil Türkçe kabul edilmiştir. | confirmed
- **ASM-02**: Output Path Category | `outputs/demos/trakya-teknik-makine/` | safe | Proje bir satış demosu/prototipi olduğu için `demo-frontend` paketi ve `demos` klasörü varsayılmıştır. | confirmed
- **ASM-03**: Existing Project Scope Boundary | `project_state: existing` | bounded | Kaynak brief'te `project_state: new` önerilmişse de, Engine kurallarına göre (`engine/PROJECT_INTAKE.md` L243) mevcut site ve korunan firma gerçekleri olduğu için `existing` sınıflandırılmıştır. | confirmed
...
- **Status:** `approved`
- **Approved At:** 2026-08-14T04:15:30+03:00
- **Approved By:** user
...
- [x] Assumption'lar kayıtlı ve status'ları doğru (`confirmed`).
...
- [x] `approved_by: user` için doğrudan approval kanıtı var ("onaylıyorum devam et").
