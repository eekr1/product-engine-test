...
updated_at: "2026-08-14T03:31:19+03:00"
approved_at: ""
approved_by: ""
supersedes: ""
...
- **Project State:** `existing` *(Not: Hedef frontend sıfırdan yazılsa da mevcut yayında olan site ve doğrulanmış firma gerçekleri korunduğu için Product Engine `PROJECT_INTAKE.md` kuralı uyarınca `existing` olarak sınıflandırılmıştır.)*
- **Delivery Profile:** `Prototype`
- **Implementation Planning:** `standard`
- **Design Planning:** `light`
- **Primary Language:** `tr`
...
- **Stack-Neutral Pending Durumu:** Kararlaştırılacak exact frontend framework/library (ör. Vanilla HTML/JS veya Vite/React vb.) explicit approval aşamasında kesinleşecektir. Intake aşamasında stack-specific kod varsayımı uydurulmamıştır.
...
- **TD-01:** Exact Frontend Stack Seçimi (Vanilla Web Standards vs Vite+React). *Approval aşamasında netleşecektir.*
- **TD-02:** İletişim formu için mock handler mı yoksa client-side mail trigger mı kullanılacağı.
...
- **DEC-03:** Delivery profile `Prototype`, implementation planning `standard`, design planning `light` olarak önerilmiştir.
...
|---|---|---|---|---|---|---|
| ASM-01 | `primary_language` | Türkçe (tr) | Safe | Firma Trakya bölgesinde yerel hizmet vermektedir ve mevcut site Türkçe'dir. | Proje Bağlamı | pending_review |
| ASM-02 | `project_type` | `web-app` | Safe | Çok bölümlü/sayfalı kurumsal frontend demo yapısı web-app kategorisine girmektedir. | Engine Standardı | pending_review |
| ASM-03 | `project_state` | `existing` | Safe | Yayında mevcut bir web sitesi ve doğrulanmış firma bilgileri mevcuttur. | Proje Bağlamı | pending_review |
| ASM-04 | `technical_stack` | Modern Web Frontend (Exact framework approval'da seçilecek) | Bounded | Demo kolay sunulabilir ve temiz service-boundary yapısına sahip olmalıdır. | Proje İhtiyacı | pending_review |
...
- **CON-02 (Prototype vs Architecture Quality):** Demo/Prototype tanımı teslim olgunluğunu sınırlar; kod ve mimari kaliteyi düşürmez. Direct component-hardcoding yasaklanmış, clean service-boundary ilkesi korunmuştur. (Çözüldü: Engine kuralı uygulandı.)

---

## 13. Open Questions / Unresolved Items

- **OQ-01 (Exact Frontend Stack):** Frontend demo için tercih edilen exact teknoloji stack'i ne olmalıdır? (Örn: Vanilla HTML5/CSS3/ES6 JS mi, yoksa React + Vite ortamı mı?)
- **OQ-02 (Logo / Visual Assets):** Firmanın yüksek çözünürlüklü vektörel logosu veya tesis/makine görselleri mevcut mu, yoksa demo için uyumlu industrial vector/imagery asset'ler mi sentezlenmeli?
- **OQ-03 (Interactive Features):** Demoda çalışır bir İletişim / Servis Talep Formu (mock submission ile) yer alsın mı?
...
- [ ] Canonical explicit user approval bekleniyor (IDE/tool/plan auto-approval kullanılmadı).
- [ ] `approved_by: user` için doğrudan approval kanıtı bekleniyor.
