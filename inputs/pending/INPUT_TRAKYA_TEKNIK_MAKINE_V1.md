...
- Teknoloji stack'i stack-neutral olarak bırakılmış olup, Product Engine package/document selection aşamasında kesinleştirilecektir.
...
UI bilesenleri veriye doğrudan hardcode edilmeyecek, bir data/service boundary üzerinden erişecektir. Bu sayede gelecekte gerçek backend API bağlandığında UI bileşenleri refactor edilmeden adapter değişimi yapılabilecektir.
...
- Kullanılacak frontend framework ve build tool seçimi (Vite + React vb.) Package selection ve Engine run aşamasında netleşecektir.
...
- [ ] Canonical explicit user approval henüz ALINMADI. (Pending intake aşamasında kullanıcı onayı bekleniyor).
- [ ] `approved_by: user` yazılamaz (Explicit user approval mesajı henüz alınmadı).
